...
export class HomePage {
	...
	slides: any[]; deal: any; products: Object[]; categories: Object[] = []; clientSay: Object[] = [];
	...
	showCoupon(){
		let coupon = this.alertCtrl.create({
			title: "Coupon: 1stBuy30",
			...
			buttons: [
				...
				},
				{
					text: "Copy",
					handler: () => {
						this.clipboard.copy('1stbuy30');
						let couponCopied = this.alertCtrl.create({
							title: "Copied to Clipboard!",
							message: "Now, you can use this coupon.",
							cssClass: 'alert-signout',
							buttons: [
								{
									text: "Ok",
	...
	openLink(url: string, external: boolean = false) {
		...
		} else if (!external) this.iab.create(url, open_target_blank ? "_blank" : "_system", "location=no");
	}
